...
## Description | 모든 샘플에서 환자×배치 pseudobulk 생성, edgeR(TMM)→limma-voom 및 edgeR QL GLM 사용
## HY-HO-DO 대비 DEG를 계산, 진단 플롯(voom/BCV)·Volcano·tsv와 batch 제거 PCA 생성
# ============================================================
...
# ============================================================
{
  # 진단: voom mean-variance
  pdf(file.path(out_limma, "voom_mean_variance.pdf"), width = 6.5, height = 5.5)
  invisible(voom(y, design, plot = TRUE))  # 이 호출은 그림만, 아래 모델은 다시 적합
  dev.off()
...
# ==========================================
{
  ...
  # 진단: BCV
  pdf(file.path(out_edger, "BCV.pdf"), width = 6.5, height = 5.5)
  plotBCV(y2)
  dev.off()

  fit2 <- glmQLFit(y2, design, robust = TRUE)
  contr_edger <- makeContrasts(
    HO_vs_DO = groupHealthy_Old   - groupDiabetes_Old,
    HY_vs_DO = groupHealthy_Young - groupDiabetes_Old,
    HO_vs_HY = groupHealthy_Old   - groupHealthy_Young,
    levels = design
  ...
  for (nm in colnames(contr_edger)) {
    qlf <- glmQLFTest(fit2, contrast = contr_edger[, nm])
  ...
  # Volcano (DEG 라벨)
  pdf(file.path(out_edger, "Volcano_edger.pdf"), width = 7, height = 6)
...
# ---- (edgeR) PCA: TMM logCPM → batch 제거 후 시각화 ----
E_edger <- cpm(y2, log = TRUE, prior.count = 0.25)

Xg <- model.matrix(~ 0 + group, data = y$samples)  # group 보호
E_edger_clean <- removeBatchEffect(E_edger, batch = y$samples$batch, design = Xg)

set.seed(42)
pca_e <- prcomp(t(E_edger_clean), scale. = TRUE)
pca_e_df <- data.frame(pca_e$x[,1:2],
                       group = y$samples$group,
                       batch = y$samples$batch,
                       sample_id = rownames(y$samples))

p_e <- ggplot(pca_e_df, aes(PC1, PC2, color = group, shape = batch, label = sample_id)) +
  geom_point(size = 3, alpha = 0.9) +
  ggrepel::geom_text_repel(size = 2.6, max.overlaps = 30) +
  labs(title = "PCA (edgeR QL, removeBatchEffect | design=~0+group)",
       subtitle = "시각화 전용: 통계는 ~0+group+batch 결과 사용",
       x = sprintf("PC1 (%.1f%%)", 100 * summary(pca_e)$importance[2,1]),
       y = sprintf("PC2 (%.1f%%)", 100 * summary(pca_e)$importance[2,2])) +
  theme_minimal(base_size = 12)
ggsave(file.path(out_edger, "PCA_edger.png"), p_e, width = 7.5, height = 6.2, dpi = 300)


message("Done. Created:\n",
        "- ", out_limma, "/voom_mean_variance.pdf + Volcano_limma.pdf + 3×DEG_*.tsv\n",
        "- ", out_edger, "/BCV.pdf + Volcano_edger.pdf + 3×DEG_*.tsv")
